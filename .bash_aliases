alias v="nvim"
alias l="ls -A"
alias pip="pip3"
alias r="ranger"
alias ..="cd .."
alias pop="popd"
alias rm="rm -i"
alias push="pushd"
alias ga="git add"
alias ll='ls -alF'
alias gp="git push"
alias ...="cd ../.."
alias btop="bashtop"
alias gs="git status"
alias gc="git commit"
alias cdmnt="cd $HDD"
alias rr="ranger $HDD"
alias python="python3"
alias sv="sudo -E nvim"
alias cco="cd $HDD/Code"
alias list="l | grep -i"
alias ....="cd ../../.."
alias dinstall="sudo dpkg -i"
alias conf="cd $HOME/.config"
alias update="sudo apt update"
alias rmo="ranger $HDD/Movies"
alias upgrade="sudo apt upgrade"
alias src="source $HOME/.bashrc"
alias ainstall="sudo apt install"
alias caps="xdotool key Caps_Lock"
alias screenkey="screenkey -s small"
alias alacritty="alacritty -t Alacritty"
alias proj="cd $HDD/Code/Attendance-Manager/; git status"
alias dufolder="du -ah --max-depth=1 | sort -rh"
alias rw="feh --bg-fill --randomize $HDD/ChrisWallpapers"
alias flame="flameshot full --path $HDD/Screenshots -d 3000"
alias android-studio="nohup studio.sh </dev/null &>/dev/null &"
alias pixel-3a="$HOME/Android/Sdk/emulator/emulator -avd Pixel_3a_API_28"
alias pixel-3xl="$HOME/Android/Sdk/emulator/emulator -avd Pixel_3_XL_API_30"
alias build="adb install $HDD/Code/Attendance-Manager/app/build/outputs/apk/debug/app-debug.apk"
alias enablenight="gsettings set org.gnome.settings-daemon.plugins.color night-light-enabled true"
alias disablenight="gsettings set org.gnome.settings-daemon.plugins.color night-light-enabled false"
alias recordscreen="ffmpeg -f x11grab -an -s hd1080 -r 25 -v:b 23000 -i :0.0 -g 1 -q:v 0.1 -vcodec libxvid $HDD/Screenshots/screenCapture.mp4"
